...
- **2D-E:** Execute approved withdrawal for eligible NPCs using Bannerlord-native travel to a
  safe friendly fortification, including existing NPC party leaders.

The current implementation includes 2D-E-03e. It preserves explicit player agency, monthly denial
deduplication, and the inactive pure battle-risk calculator. Approved NPC party members can leave
field service through native delayed travel. Approved NPC party leaders keep their existing party
for the trip, leave any army, physically travel to protection, and retire through Bannerlord's
native disband-to-fortification lifecycle on arrival. The approved mother remains unavailable for
normal party leadership/army service until that pregnancy ends. Automatic player-character
movement, escort-size troop splitting, and pregnancy-loss behavior remain deferred.
...
39. Voluntary continuation preserves physical injury risk but assigns no commander relationship
    target.
40. An approved eligible ordinary NPC begins Bannerlord native delayed travel exactly once.
41. An approved NPC party leader leaves any army and physically travels with her existing party.
42. Save/load while native travel is active does not start a second trip.
43. Arrival at the selected friendly fortification records protected rest and completes execution.
44. Native party-leader retirement merges the party through Bannerlord's fortification-disband
    lifecycle rather than generic party deletion.
45. A canceled/invalid native trip remains pending or can retry without duplicating execution.
46. Player-character movement is not automated in 2D-E.
47. An approved pregnant NPC remains ineligible for normal party leadership/army service after
    arrival until the pregnancy ends.
48. If a restricted withdrawal party is nevertheless re-added to an army, PLE removes it again
    through the native army path and reasserts the withdrawal route.
49. Completing the travel execution never downgrades the persisted state back to Pending.
50. Party-leader travel permits Bannerlord land/naval mixed routing rather than forcing land-only
    navigation.
51. A restricted party that reaches its protected fortification retires on a safe party tick without waiting for the next daily pregnancy observation.
52. Re-adding a restricted party from a native conversation cannot retire/destroy that party inside the conversation callback; the join is reversed immediately and route/arrival execution is deferred to the safe party tick.
...
## Milestone 2D-E-03e — Safe Rejoin Cleanup, Leader-Only Army Disband, and Pregnancy-Long Service Restriction

- Player withdrawal petitions use a native `MultiSelectionInquiryData` with `isExitShown=false`; a petition requires one explicit selection and Escape is not treated as denial.
- Approved NPC party members continue to use Bannerlord delayed hero travel.
- An approved pregnant NPC who leads her own mobile party keeps that party for the withdrawal trip, leaves any army through the native `Army` removal path, and receives Bannerlord's native settlement-travel order to the selected safe fortification.
- Party-leader travel uses `MobileParty.NavigationType.All`, allowing Bannerlord to select land, naval, or mixed routing instead of PLE forcing a land-only route.
- `CanHeroLeadPartyEvent` is vetoed for an approved pregnant NPC from approval until that pregnancy ends. `Completed` means the withdrawal trip is finished; it does not restore field-service eligibility.
- `OnPartyJoinedArmyEvent` is a second enforcement layer. If a restricted withdrawal party is nevertheless re-added to an army, PLE removes it again through the native army path, but does not retire/re-route the party or disband the remaining army inside that native callback. Route reassertion and army cleanup are deferred to the next safe hourly party tick.
- If the native rejoin/removal sequence leaves a stale `AttachedTo` reference, the safe hourly tick clears it through Bannerlord's public attachment property before travel is reasserted. This keeps the army leader's `AttachedParties` collection and displayed troop strength from retaining the withdrawn party as a ghost.
- A leader-only army is auto-disbanded **only when PLE's removal of the approved pregnant withdrawal party caused that leader-only state**. PLE queues the original army and, on the next safe hourly tick, calls Bannerlord's native `DisbandArmyAction.ApplyByUnknownReason` only if the leader still has zero attached parties. If another party joined meanwhile, the queued cleanup is canceled. PLE does not globally disband legitimate one-party/gathering armies.
- Arrival completion is checked on a safe hourly party tick so a party that has physically reached its protected fortification no longer waits for the next daily pregnancy observation. On arrival, PLE uses Bannerlord's disbanding-specific fortification lifecycle. Vanilla `OnPartyDisbanded` handling merges regular troops, wounded troops, troop XP and appropriate prisoners into the garrison and places living heroes in the settlement before Bannerlord retires the mobile party. PLE does not use generic party destruction for this withdrawal path.
- Successful native retirement immediately commits the execution ledger to `Completed`, preventing the prior false `Completed -> Pending` diagnostic.
